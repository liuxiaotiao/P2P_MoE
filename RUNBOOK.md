...
**各台磁盘占用不一样** —— 那是正常的。清单里每台的份额本就不同（0.9 GB 到
22.4 GB，按层区间与驻留集算出来的）。`./deploy_15.sh disk` 把**份额、实占、幽灵**
三个数摆在一起，差异对不对得上一眼就看得出来。
...
| `./deploy_15.sh disk` | 控制机 | 逐台磁盘：份额 vs 实占 vs 幽灵，一张表 |
